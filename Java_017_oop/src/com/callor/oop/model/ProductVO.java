...
public class ProductVO {
	...
	/*
	 * java에서는 class 를 선언하면 public String toString() 매서드가 자동으로 생성된다 하지만 기본적으로 코드는
	 * 보이지 않는다 toString() 매서드는 객체를 생성했을때 어떤 클래스를 사용했는지와 생성된 객체가 컴퓨터 기억장치의 어떤 곳(주소)에
	 * 만들어져 저장되어 있는지 알려주는 코드가 담겨있다 java 코딩에서 toString()매서드의 기본 역활은 크게 필요하지 않다. 그래서
	 * 일반적으로 VO 클래스를 만들때는 임의로 toString() 매서드를 "다시 만들어준다" 그리고 인스턴스 변수에 입력돤 값을 알려주는
	 * 디버깅 코드를 생성한다
	 * 
	 * method Override ( Overriding ) 매서드의 재정의 (java)가 자동으로 만들어져 있는 toString() 개발자가
	 * 임의로 다시 만들었다.
	 * 

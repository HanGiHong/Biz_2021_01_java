...
public class MenuServiceImplV3 implements MenuService{
   ...
   /*
    * 메뉴를 보여주고
    * 선택된 메뉴에 따라서 카트에 추가, 삭제 하거나
    *       리스트를 보여줘야한다.
    *       이기능을 MenuService 에서 구현하는것이아니라
    *       selecetMenu()를 호출한 곳에서 구현을 해야한다.
    * 
    ...
    *       void type이 아닌 int나 String 등 다른 type 으로 변경해야한다.
    *       클래스 method를 변경하려면 
    *       interface 부터 변경하고, interface를 implements한 
    *       모든 클래스를 다 변경해야한다.
    * 
    * interface, class의 원형을 변경하지 않고
    *       현재 method 에만 어떤 기능을 추가하고자 한다.
    * 
